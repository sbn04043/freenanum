...
public class ProductServiceImpl implements ProductService {
    ...
    @Override
    public List<ProductModel> findProductsByUserId(Long userId) {
        List<ProductEntity> products = productRepository.getByUserId(userId);

        // 상품 목록을 ProductModel로 변환하고, 첫 번째 이미지 URL을 추가
        return products.stream()
                .map(productEntity -> {
                    ProductModel productModel = ProductModel.toModel(productEntity);

                    // 첫 번째 이미지 가져오기
                    String firstImageUrl = productImageRepository.getOneById(productEntity.getId());
                    productModel.setImgUrl(firstImageUrl);  // 첫 번째 이미지 URL을 모델에 추가
