...
public class ChatRestController {
    ...
    public Mono<ChatMessageEntity> setChatMessageReadTrue(@PathVariable String chatMessageId) {
        return chatMessageRepository.findById(chatMessageId)  // DB에서 메시지 가져오기
                .flatMap(chatMessageEntity -> {
                    if (chatMessageEntity != null) {
                        chatMessageEntity.setRead(true);  // 메시지 읽음 상태로 변경
                        return chatMessageRepository.save(chatMessageEntity);  // 변경된 엔티티를 DB에 저장
                    } else {
                        return Mono.empty();  // 메시지가 없으면 빈 Mono 반환
                    }
                });
    }
